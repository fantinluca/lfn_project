...
\section{Subrgraphs creation and analysis}
In the implemented code, subgraphs are being generated with \texttt{networkx} and \texttt{networkit} packages.
In \texttt{subgraphGenre.py} a subgraph is computed by considering only the nodes which represent artists of a certaing musical genre. The subgraph is being created by considering only the existing edges between those nodes.
The code computes degree, closeness,  betweenness, PageRank, eigenvector centrality, local, global and approx global clustering coefficient.
It also stores the subgraph in a suitable file in order to be analyzed with the SILVAN algorithm.
